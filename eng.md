**A note from the editors:** We are pleased to present an excerpt from Chapter 5
...
`“for”` attribute, and the input field’s `“id”` attribute making the connection
.</figcaption></figure>
...
way that software can read, so it is accessible to users of assistive technology.

For example, one interaction pattern is the “accordion” widget, which is a
link that, when clicked, expands to show hidden content (see Figure 5.2). 
Clicking a second time collapses the content back to its hidden state. This 
pattern is helpful for content that may not be relevant to all users. It saves 
precious screen real estate, and also provides a way to learn more in context, 
without jumping to different pages or scrolling through one long page.
...
![Figure 5.2: Example accessible accordion widget from OpenAjax Alliance][3]<
figcaption
>**Figure 5.2**: The [OpenAjax Alliance][4] provides examples and downloadable
code for many common design patterns, including an
[accessible accordion widget][5].</figcaption></figure>
...
screen reader users.<figure>

![Figure 5.3: Account sign-up form error message][6]<figcaption>**Figure 5.3**
